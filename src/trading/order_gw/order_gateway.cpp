...
namespace Trading {
    ...
    logger_("trading_gateway_" + std::to_string(client_id) + ".log"),
    tcp_socket_(logger_) {
    ...
    auto OrderGateway::recvCallback(TCPSocket *socket, const Nanos rx_time) noexcept -> void {
        ...
        if (socket -> next_rcv_valid_index_ >= sizeof(Exchange::OMClientResponse)) {
            ...
            for ( ; i + sizeof(Exchange::OMClientResponse) <= socket -> next_rcv_valid_index_; i += sizeof(Exchange::OMClientResponse)) {
                const auto response = reinterpret_cast<const Exchange::OMClientResponse *> (socket -> inbound_data_.data() + i);
                ...
                if (response -> me_client_response_.client_id_ != client_id_) {
                    logger_.log("%:% %() % ERROR. Incorrect client id. Client id expected: %, received: %. \n",
                        __FILE__, __LINE__, __func__,
                        getCurrentTimeStr(&time_str_),
                        client_id_,
                ...
                if (response -> seq_num_ != next_exp_seq_num_) {
                    logger_.log("%:% %() % ERROR. Incorrect sequence number. Client id: %, Seq num expected: %, Received: %. \n",
                        __FILE__, __LINE__, __func__,
                        getCurrentTimeStr(&time_str_),
                        client_id_,
                        next_exp_seq_num_,
