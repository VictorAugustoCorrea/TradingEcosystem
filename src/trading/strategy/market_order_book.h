...
#ifndef TRADINGECOSYSTEM_MARKET_ORDER_BOOK_H
#define TRADINGECOSYSTEM_MARKET_ORDER_BOOK_H
...
namespace Trading {
    ...
    private:
        ...
        auto addOrdersAtPrice(MarketOrdersAtPrice *new_orders_at_price) noexcept {
            ...
            if (const auto best_orders_by_price = new_orders_at_price -> side_ == Side::BUY ? bids_by_price_ : asks_by_price_;
        ...
        auto addOrder(MarketOrder *order) noexcept -> void {
            if (const auto orders_at_price = getOrdersAtPrice(order -> price_); !orders_at_price) {
                ...
                const auto new_orders_at_price = orders_at_price_pool_.allocate(order -> side_, order -> price_, order, nullptr, nullptr);
                addOrdersAtPrice(new_orders_at_price);
            } else {
                const auto first_order = orders_at_price -> first_mkt_order_;
...
#endif //TRADINGECOSYSTEM_MARKET_ORDER_BOOK_H