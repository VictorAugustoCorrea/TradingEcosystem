...
namespace Trading {
    ...
    auto MarketOrderBook::onMarketUpdate(const Exchange::MEMarketUpdate *market_update) noexcept -> void {
        ...
        switch (market_update -> type_) {
            ...
            case Exchange::MEMarketUpdateType::INVALID:
            case Exchange::MEMarketUpdateType::SNAPSHOT_START:
            case Exchange::MEMarketUpdateType::SNAPSHOT_END:
            break;
        }
    ...
    auto MarketOrderBook::toString(const bool detailed, const bool validity_check) const -> std::string {
            ...
            for (auto o_itr = itr -> first_mkt_order_; ; o_itr = o_itr -> next_order_) {
                if (detailed) {
                    sprintf(buf, "[oid:%s q:%s p:%s n:%s] ",
                        orderIdToString(o_itr -> order_id_).c_str(),
...

        {
            ...
            for (size_t count = 0; ask_itr; ++count) {
                ss << "ASKS L:" << count << " => ";
                const auto next_ask_itr = ask_itr -> next_entry_ == asks_by_price_
                    ? nullptr
                    : ask_itr -> next_entry_;
...

        {
            ...
            for (size_t count = 0; bid_itr; ++count) {
                ss << "BIDS L:" << count << " => ";
                const auto next_bid_itr = bid_itr -> next_entry_ == bids_by_price_
                    ? nullptr
                    : bid_itr -> next_entry_;
