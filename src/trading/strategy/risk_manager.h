...
#ifndef TRADINGECOSYSTEM_RISK_MANAGER_H
#define TRADINGECOSYSTEM_RISK_MANAGER_H
...
namespace Trading {
    ...
    inline auto riskCheckResultToString(const RiskCheckResult result) {
        switch (result) {
    ...
    struct RiskInfo {
        ...
        auto toString() const {
            std::stringstream ss;
            ss  << "RiskInfo: [ "
                << "Position: " << position_info_ -> toString() << " "
                << risk_cfg_.toString() << ". ]";
            return ss.str();
        }
    };
...
#endif //TRADINGECOSYSTEM_RISK_MANAGER_H
