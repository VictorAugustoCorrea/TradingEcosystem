...
void TradeClientApplication::start() {
    ...
    if (!use_shared_queues_) {
        /**
         * Network mode: receive market data from the exchange via multicast.
         * The MarketDataConsumer feeds the same market_updates queue that
         * TradeEngine reads from, but now populated over the network.
         */
    ...
    /** Allow the engine to stabilise before trading */
    usleep(10 * 1000 * 1000);
